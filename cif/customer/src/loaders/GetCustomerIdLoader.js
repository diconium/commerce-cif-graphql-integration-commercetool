...
const CustomerSignInMutation = require('../graphql/customerSignIn.graphql');

class GetCustomerIdLoader {
  /**
   ...
   */
  constructor(graphqlContext, actionParameters) {
    // The loading function: "input" is an Array of parameters
    let loadingFunction = accessTokens => {
      return Promise.resolve(
        accessTokens.map(input => {
   ...
   */
  _getCustomerId(graphqlContext, actionParameters, input) {
    return new Promise((resolve, reject) => {
      const { email, password } = graphqlContext;
      ...
      axios
        .request(request)
        .then(response => {
          resolve(input + '+' + response.data.data.customerSignIn.customer.id);
        })
        .catch(error => {
          reject(error);
        });
...
/**
 * @type {GetCustomerIdLoader}
 */
module.exports = GetCustomerIdLoader;
