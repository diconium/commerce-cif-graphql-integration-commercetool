...
const CreateEmptyCartMutation = require('./../graphql/createEmptyCart.graphql');
class CreateEmptyCartLoader {
  /**
    ...
    let loadingFunction = inputs => {
      return Promise.resolve(
        inputs.map(input => {
          ...
          return this._createEmptyCart(input, actionParameters).catch(error => {
   ...
   * @private
   */
  _createEmptyCart(input, actionParameters) {
    return new Promise((resolve, reject) => {
      ...
      axios
        .request(request)
        .then(response => {
          resolve(response.data.data.createMyCart);
        })
        .catch(error => {
          reject(error);
        });
...
/**
 * @type {CreateEmptyCartLoader}
 */
module.exports = CreateEmptyCartLoader;
