...
const PlaceOrderMutation = require('../graphql/placeOrder.graphql');
...
class PlaceOrderLoader {
  ...
  /**
   * Method used to place order from cart
   ...
   * @returns {Promise} a promise resolves and creates the order from cart.
   */
  _placeOrder(input, version, graphqlContext) {
      ...
      axios
        ...
        .then(response => {
          if (!response.data.errors) {
            return resolve(response.data.data.createOrderFromCart);
          }
